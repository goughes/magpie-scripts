...
for( blocksize in blocksize.c){
  ### 1. Simulate Data 
  for(m in m.c){
    dir.dm  <- paste(rawnetworkhome,"results/","m",m,"/",sep="")
    print(dir.dm)
    if (!dir.exists(dir.dm)) {
	 print("checked out dir.dm")
         tempdir<- dir.create(dir.dm, recursive = TRUE)
	 print(tempdir)
    }
    # stop("stop after creation")
    mr1 <- rhwatch(
      map      = map1,
      input    = c(2^(n-m),2),
      output   = dir.dm,
  ...
  for( rep in rep.c ){
    for(m in m.c){
      ...
      dir.nf  <- paste(rawnetworkhome,"Otime/","m",m,"/",sep="")
      if (!dir.exists(dir.nf)) {
           dir.create(dir.nf, recursive = TRUE)
      }
      mr2 <- rhwatch(
        map      = map2,
        input    = dir.dm,
    ...
    for(m in m.c){
      ...
      dir.gf  <- paste(rawnetworkhome,"Ttime/","m",m,"/",sep="")
      if (!dir.exists(dir.gf)) {
           dir.create(dir.gf, recursive = TRUE)
      }
      mr3 <- rhwatch(
        map      = map3,
        reduce   = reduce3,
...
save(timing, file="performance_Raw.RData")
#---------------------------------------------------------------------
